...
def validate_config(config):
    ...
    return print("Config loaded successfully")
...
def load_config(config_file_path):
    ...
    # Don't remove this as will help you doing some basic checks on config content
...
def get_class_names(config):
    ...
    """
    # sorted because of 16.06 code review
...
def predict_from_folder(folder, model, input_size, class_names):
    ...
    # You can use os.walk() or walkdir() to iterate over the files in the folder.
    for dirpath, filename in walkdir(folder):
        image_path = os.path.join(dirpath, filename)
        # Use keras.utils.load_img() to correctly load the image
        image = keras.utils.load_img(image_path, filename)
        # Use keras.utils.img_to_array() to convert it to the format needed
        image = keras.utils.img_to_array(image)
        # Resize images to keras model input_size
        image = keras.preprocessing.image.smart_resize(image, input_size)
        # Reshape (32, 224, 3) to (None, 224, 224, 3) expected for predict
        image = image.reshape([1, 224, 224, 3])

        # get predictions
        pred = model.predict(image)
        # calculate highest probability of pred array
        pred_max = pred.argmax()
        # get highest probability of class_names and append to predictions list
        pred_class = class_names[pred_max]
        predictions.append(pred_class)

        # get label name and append to list
