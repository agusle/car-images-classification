...
# all values in created model assigned by default
def create_model(
    ...
    if weights == "imagenet":
        ...
        if data_aug_layer != None:
            ...
            data_aug_layer = data_augmentation(input)
        else:
            data_aug_layer = input
        ...
        preprocess_layer = preprocess(data_aug_layer)
        ...
        outputs = classification(dropout_layer)

        # Now you have all the layers in place, create a new model
        # Use keras.Model()
        # Assign it to `model` variable

        model = keras.Model(inputs=input, outputs=outputs, name="AgusNet")
    else:
